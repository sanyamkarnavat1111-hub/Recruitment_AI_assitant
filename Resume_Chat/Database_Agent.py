from llm import llm_sql_query_generator , llm_sql_fixer
...
class SQLAgent:
    ...
    def generate_sql_query(self, thread_id: str , chat_history : list[str] , user_query : str) -> str:
        try:
            ...
            sql_generator_prompt = PromptTemplate.from_template("""You are an expert SQLite3 query generator for a recruitment AI assistant.

            Your job: generate ONE perfect, executable SELECT query that answers the user's latest question using data stored in the database.

            ### CRITICAL CONTEXT RESOLUTION RULES (same strength as top-tier RAG rewriters):
            1. Always study the full conversation history first.
            2. Resolve ALL pronouns and vague references ("he", "she", "her", "him", "that candidate", "the previous one", "the shortlisted guy", "the one with React", etc.) and replace with the actual name, email, location, skill mentioned earlier in conversation history if available.
            3. If the user says "the last candidate", "the one we just discussed", "the current resume" → use the most recently mentioned candidate in history.
            4. Never hallucinate data that isn't in the history.
            5. If no specific candidate is identifiable  fall back to querying all candidates for this thread_id and let the LLM decide later.

            ### DATABASE RULES (must always obey):
            - READ-ONLY: Only SELECT queries allowed.
            - Always include WHERE thread_id = '{thread_id}' (exact match).
            - For text columns (candidate_name, email_address, location, skills, etc.):
                - Use LOWER(column) to convert search term to lowercase in the query and use LIKE clause for pattern matching
            - Prefer fuzzy matching over exact matches for names/emails.
            - You may JOIN `users` and `job_description` when needed.
            ...
            ### Here is the Thread ID that has to be used compulsarily:
            {thread_id}
            """)
            
            
            sql_query_generator = sql_generator_prompt | llm_sql_query_generator
